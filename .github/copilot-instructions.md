...
When refreshing this repository, run M-PACT from the repository root. If `scripts\build-refresh-bundle.js` exists in the repo, use:

```powershell
node scripts\build-refresh-bundle.js
```

Then read/apply the `BundlePath` from the manifest. Do not run refresh by changing directory into `.codex\skills\m-pact`, `.claude\skills\m-pact`, `.copilot\skills\m-pact`, or any other skill install folder. A `project: (none found)` result from a skill install folder is invalid for this repo and must not trigger bootstrap.
