...
2. Run refresh again so the project ID enters the receipt.
3. Retry the original durable write only after refresh supplies the project ID, passing it as `--project-id <n>`.

Example:

```bash
node <this-skill>/scripts/adopt-project-identity.js --root .AgentMemory
```
