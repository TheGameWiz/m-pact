...
- Task creation: Director-gated; owner `references/create-task.md`; writes the `A__` folder and `task.md`; likeliest mistake: computing the task number or treating the creator as task authority; the helper assigns numbers and no role seats are created. Explicit Director phrases like "make this a task" birth an ordinary task from the live conversation. Standalone handoff phrases such as "handoff," "hand this off," or "handoff to <agent>" naming no existing task ask one short confirmation before task creation; the phrase boundary is owned by `references/take-task-handoff.md`.
