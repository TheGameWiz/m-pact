...
- When taking a handoff or resuming task context, always read `task.md` and read `specification.md` when present, then list log filenames with sizes before deciding how much log history to load. The reader's last known record is a valid read cursor, but this read cursor does not authorize stale write numbering, implementation, specification edits, or log appends. A known read cursor or latest relevant current-state/handoff summary boundary takes priority for deciding the current handoff span: read every later record except exact known self-written identities in non-colliding numeric-prefix groups; if the later unread span is large, use the newest-backward 50KB budget unless the Director requested the full span. Older records before the cursor or summary boundary may still be read as background context when cheap enough or directly relevant, but they are not the current handoff span and must not resurrect resolved issues as current. If no cursor or summary boundary exists and total log bytes are 50KB or less, read all log records. If no cursor or summary boundary exists, total log bytes exceed 50KB, and the Director did not request all logs or a specific range, read newest records backward up to about 50KB of log content, including the newest single log even if it alone exceeds 50KB. If a numeric prefix has multiple files in the selected span, read every file in that collision group, including any entry the agent thinks it wrote, and report the duplicate record number. Report what log range was loaded as the current span, what cursor or summary boundary was used, and what older history was loaded only as background or skipped; note obvious topic boundaries rather than pretending un-read logs were loaded. Treat older loaded records as historical background when later records, `specification.md`, or the Director have superseded them.
