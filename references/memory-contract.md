...
After successful refresh, the verified bundle is the loaded startup context. Agents must not read `.AgentMemory`, `.AgentMemoryRoot`, rules, sessions, tasks, journals, case studies, generated temp bundles, or other memory artifacts merely to verify refresh, prove the bundle, or continue startup. The script already selected startup content and audited the bundle. Additional memory reads after refresh require a targeted lookup need: explicit Director request, a relevant current task need, or a `LimitHit: true` partial-bundle gap that matters to the work.
