...
6. If a `tasks/current__<task-folder>` sentinel points at the closed task, remove the sentinel unless the Director explicitly names a valid active replacement current task. Never infer a replacement from other active tasks, and never leave the pointer aimed at a closed folder.
