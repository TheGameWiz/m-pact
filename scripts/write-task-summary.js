...
const { appendMember, listMembers, readMember } = require("./lib/zip-record-store");
const { withDirectoryLock } = require("./lib/directory-lock");
const { localTimestamp, memberName, nextRecordNumber, resolveTaskPath, runCli } = require("./lib/helper-common");
...
function main({ args, input }) {
  ...
  return withDirectoryLock(taskPath, () => {
    ...
    const record = nextRecordNumber(listMembers(zipPath));
    ...
    return { ok: true, operation: "write-task-summary", taskPath, zipPath, record, member, timestamp: timestamp.body };
  });
}
